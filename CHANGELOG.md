...
### Added
- **TLS/SSL support** (P1): opt-in client-side TLS for connecting to SSL-enabled CUBRID brokers (`SSL=ON`). Set `ssl: true` on `createClient` for default certificate verification, or pass a `ClientSSLOptions` object (`rejectUnauthorized`, `ca`, `servername`) for self-signed / private-CA certificates. The client performs a STARTTLS-style upgrade on the same broker port: it sends the SSL client magic (`CUBRS`), reads the single CAS redirect/status acknowledgement, then upgrades the socket to TLS (minimum TLS 1.2, `rejectUnauthorized: true` by default) before the database login. The plaintext path is unchanged when `ssl` is omitted. Validated end-to-end against a live `SSL=ON` CUBRID 11.2 broker (`SELECT` round-trips over TLS 1.3).
...
- **Concurrent-query race condition on the shared connection** (#39): a single `CASConnection` is shared across all non-transactional queries, but `connect()`, `sendAndRecv()`, and `close()` mutated shared socket / receive-buffer / `CAS_INFO` state with no serialization. Overlapping calls (e.g. `Promise.all([...])`) could interleave — double-connecting, corrupting the length-framed wire protocol, or tearing down the socket mid-response. All three public entry points now run through a per-instance FIFO queue so operations execute strictly one-at-a-time; concurrent first-use `connect()` calls collapse to a single physical connection, and `close()` waits for any in-flight request instead of destroying the socket underneath it.
- **TLS connect deadlock against live `SSL=ON` brokers**: the SSL handshake hung forever on connect. `upgradeToTls()` read a second plaintext 4-byte "NO_ERROR" int after the redirect/status int, but a real CUBRID CAS sends only **one** plaintext int and then immediately runs `SSL_accept` — so the client blocked waiting for bytes that never arrive while the server blocked waiting for the TLS ClientHello. The client now issues the TLS handshake immediately after the single redirect/status int (verified against a live `SSL=ON` CUBRID 11.2 broker). The TLS mock broker in the test suite was corrected to the real one-int ordering, which had masked the bug.
